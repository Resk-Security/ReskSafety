# Agent setup for `Resk`

This is a deepsec scanning workspace. Project `Resk` was just registered
(target: `..`). Setup is incomplete — `data/Resk/INFO.md`
...
2. **Fill in `data/Resk/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id Resk
pnpm deepsec process --project-id Resk
```
