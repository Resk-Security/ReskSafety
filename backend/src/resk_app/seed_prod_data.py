...
USERS = {
    ...
    "carol": {
        "email": "carol@resk.sec",
        "password": "carol123",
        "is_admin": False,
        "role": "analyst",
    },
    "mallory": {
        "email": "mallory@resk.sec",
        "password": "mallory123",
        "is_admin": False,
        "role": "restricted",
    },
...
POLICIES = {
    "default-security": {
        ...
        "classifiers": {
            "jailbreak_patterns": _DEFAULT_JAILBREAK_PATTERNS,
        },
        "rules": [
            {
...
def _seed_provider(session) -> None:
    existing = session.scalar(select(Provider).where(Provider.name == "DeepSeek"))
    if existing:
        return
